...
;;;
;;; accumulator-definitions.rkt
;;;
;;; Provides several accumulator implementations.
;;;
...
(define-syntax (to-list stx)
  (syntax-parse stx
    [(_ (~optional (~seq #:reverse? reverse?:expr)
                   #:defaults ([reverse? #'#f])))
     (make-accumulator
      #:loop-bindings #'([accumulator '()])
      #:body-outputs #'(body-output)
      #:recurse-arguments #'((cons body-output accumulator))
      #:return #'(if reverse? accumulator (reverse accumulator)))]))
